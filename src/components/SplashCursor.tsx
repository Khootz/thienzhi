...
// Fullscreen fluid splash cursor overlay
const SplashCursor = ({
  SIM_RESOLUTION = 128,
  DYE_RESOLUTION = 1440,
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      let gl: any = (canvasEl.getContext('webgl2', params) as any) || null;
      const isWebGL2 = !!gl;
      if (!isWebGL2)
        gl =
          (canvasEl.getContext('webgl', params) as any) ||
      ...
      function getSupportedFormat(glX: any, internalFormat: any, format: any, type: any) {
        if (!supportRenderTextureFormat(glX, internalFormat, format, type)) {
          switch (internalFormat) {
            case glX.R16F:
              return getSupportedFormat(glX, glX.RG16F, glX.RG, type);
            case glX.RG16F:
              return getSupportedFormat(glX, glX.RGBA16F, glX.RGBA, type);
            default:
              return null;
          }
        }
      ...
      function supportRenderTextureFormat(glX: any, internalFormat: any, format: any, type: any) {
        ...
        glX.texImage2D(glX.TEXTURE_2D, 0, internalFormat, 4, 4, 0, format, type, null);
        const fbo = glX.createFramebuffer();
        glX.bindFramebuffer(glX.FRAMEBUFFER, fbo);
        glX.framebufferTexture2D(glX.FRAMEBUFFER, glX.COLOR_ATTACHMENT0, glX.TEXTURE_2D, texture, 0);
    ...
    function compileShader(type: any, source: string, keywords?: string[] | null) {
      ...
      if (!gl.getShaderParameter(shader, gl.IMPLEMENTATION_COLOR_READ_TYPE)) {
        // no-op
      }
    ...
    document.body.addEventListener('touchstart', handleFirstTouchStart as any);
